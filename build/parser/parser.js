        ...
        ast.rules.forEach((rule) => {
            if (rule.type === 'ruleset') {
                ...
                rule.rulelist.rules.forEach((v) => {
                    ...
                    if (v.type === shady_css_parser_1.nodeType.declaration && v.value ? .type === shady_css_parser_1.nodeType.expression : ) {
                        rulelist[v.name] = v.value.text;
                    }
                });
                result[rule.selector] = rulelist;
            }
            else if (rule.type === 'declaration' && rule.value ? .type === shady_css_parser_1.nodeType.expression : ) {
