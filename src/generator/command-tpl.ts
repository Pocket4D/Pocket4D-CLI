...
export const createTemplate = (tplcode: string, tplname: string) => {
  let projectDir = path.resolve("."),
    clidir = __dirname,
    tplDir = path.resolve(clidir, "tpl");
  ...
  logger.success(
    `create template cdoe :${tplcode}    name:${tplname} successfully`
  );
};
