...
export class P4DParser {
		...
		ast.rules.forEach((rule) => {
			if (rule.type === 'ruleset') {
				...
				rule.rulelist.rules.forEach((v) => {
					...
					if (v.type === nodeType.declaration && v.value?.type === nodeType.expression) {
						rulelist[v.name] = v.value.text;
					}
				});

				result[rule.selector] = rulelist;
			} else if (rule.type === 'declaration' && rule.value?.type === nodeType.expression) {
	...
	_handleConfig() {
		...
		if (fs.existsSync(configFile)) {
			this.result['config'] = JSON.parse(fs.readFileSync(configFile).toString());
		}
	}
